...
{	
	...
	*/
	...
	function save_accident(&$person_data, &$customer_data,$customer_id=false)
	{
		$success=false;
		//Run these queries as a transaction, we want to make sure we do all or nothing
		$this->db->trans_start();
		...
		//create encouter and give the patient an encounter status of 3
		//$this->db->select('person_id');
		$this->db->where('department_created', 'accident');
		$this->db->order_by('person_id','desc');
		$this->db->limit(1);
		$this->db->from('mlkh_people');
		$patient_id=$this->db->get()->row()->person_id;
		/*if ($q->num_rows() > 0) {
		...
		//$query1=$this->db->query("SELECT `person_id` FROM `mlkh_people` WHERE `department_created`='accident' ORDER BY `person_id` DESC");
		//$patient_id=$query1->row();
		$visit_status='0';
		$encounter_type="accident";
		$encounter_status='1';
		$encounter_data = array(
			'patient_id' => $patient_id,
			'visit_status' =>$visit_status,
			'encounter_type' => $encounter_type,
			'encounter_queue' => $encounter_type,
