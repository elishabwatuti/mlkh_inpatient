...
class Sale_lib
{
	...
	//bwats section_queue
	function get_section_queue() 
	{
		return $this->CI->session->userdata('section_queue');
	}

	function set_section_queue($section_queue) 
	{
		$this->CI->session->set_userdata('section_queue', $section_queue);
	}
	//end of the section_queue
	...
	function add_item($item_id,$quantity=1,$discount=0,$price=null,$description=null,$serialnumber=null,$invoiced=null)
	{
		...
		{
			if(!$invoiced)	$items[$updatekey]['quantity']+=$quantity;
		}
		else
		{
			//add to existing array
