...
<div id="page_title" style="margin-bottom:8px;"><?php echo $this->lang->line('invoices_service_invoice'); ?></div>
<?php
if(isset($error))
{
...
else
{
	foreach(array_reverse($cart, true) as $line=>$item)
	{
		...
		echo form_open($controller_name."/edit_item/$line");
	?>
		<tr>
...
<div id="overall_sale">
	...
	if(isset($customer))
	{
		...
		echo anchor($controller_name."/remove_customer",'['.$this->lang->line('common_remove').' '.$this->lang->line('customers_customer').']');
	}
	else
	{
	...
	?>
		...
		?>
			<div id="finish_sale">
				<?php echo form_open($controller_name."/complete",array('id'=>'finish_invoice_form')); ?>
        ...
        <div id="Cancel_sale">
		<?php echo form_open($controller_name."/cancel_invoice",array('id'=>'cancel_sale_form')); ?>
...
$(document).ready(function()
{
	...
	$("#suspend_sale_button").click(function()
	{
    	...
    	{
			$('#finish_invoice_form').attr('action', '<?php echo site_url($controller_name."/suspend"); ?>');
    		$('#finish_invoice_form').submit();
    	}
	});
