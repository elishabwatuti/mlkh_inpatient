...
<div id="page_title" style="margin-bottom:8px;"><?php echo $this->lang->line('invoices_service_request'); ?></div>
<?php
if(isset($error))
{
...
<?php echo form_open($controller_name."/add_service_item",array('id'=>'add_item_form')); ?>
...
{
	foreach(array_reverse($service_cart, true) as $line=>$item)
	{
		$cur_item_info = $this->Item->get_info($item['item_id']);
		echo form_open($controller_name."/service_request");
	?>
		<tr>
		...
		<td><?php echo anchor($controller_name."/delete_service_item/$line",'['.$this->lang->line('common_delete').']');?></td>
		</tr>
...
<table>
	...
	<div id='sale_details'>
		...
		if(isset($customer) && count($service_cart) > 0)
		{
		?>
				<?php echo form_open($controller_name."/complete_service_request",array('id'=>'finish_invoice_form')); ?>
                <label id="opd_services_label" for="opd_services"><?php echo "Outpatiens Services to be Carried Out"; ?>:</label>

				<input type="checkbox" name="opd_services[]" value="1">&nbsp;Plaster of Paris<br/>
        		<input type="checkbox" name="opd_services[]" value="1">&nbsp;Stitching<br/>
        		<input type="checkbox" name="opd_services[]" value="1">&nbsp;Dressing<br/>
        		<input type="checkbox" name="opd_services[]" value="1">&nbsp;Injection<br/>
        		<br /><br />
		...
		?>
                <?php echo form_open($controller_name."/cancel_service_request",array('id'=>'cancel_sale_form')); ?>
...
{
    $("#item").autocomplete('<?php echo site_url($controller_name."/service_item_search"); ?>',
    {
    	minChars:0,
    	max:100,
    	selectFirst: false,
	...
	{
		$.post('<?php echo site_url("service_request/set_comment");?>', {comment: $('#comment').val()});
	});
	
	$('#email_receipt').change(function() 
	{
		$.post('<?php echo site_url("service_request/set_email_receipt");?>', {email_receipt: $('#email_receipt').is(':checked') ? '1' : '0'});
	});
	...
	$("#suspend_sale_button").click(function()
	{
    	...
    	{
			$('#finish_invoice_form').attr('action', '<?php echo site_url("service_request/suspend"); ?>');
    		$('#finish_invoice_form').submit();
    	}
	});
