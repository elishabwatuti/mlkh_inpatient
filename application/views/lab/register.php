...
<div id="page_title" style="margin-bottom:8px;"><?php echo $this->lang->line('invoices_lab'); ?></div>
<?php
if(isset($error))
{
...
<?php echo form_open("lab/add",array('id'=>'add_item_form')); ?>
...
else
{
	foreach(array_reverse($cart, true) as $line=>$item)
	{
		...
		echo form_open("lab/edit_item/$line",array('id'=>'edit_item_form'));
	?>
		<tr>
		...
		<td><?php anchor("lab/delete_item/$line",'['.$this->lang->line('common_delete').']');?></td>
		</tr>
...
<div id="overall_sale">
	...
	if(isset($customer))
	{
		...
		echo anchor("lab/remove_customer",'['.$this->lang->line('common_remove').' '.$this->lang->line('customers_customer').']');
	}
	else
	{
		echo form_open("lab/select_customer",array('id'=>'select_customer_form')); ?>
	...
	?>
		...
		?>
			<div id="finish_sale">
				<?php echo form_open("lab/save_incomplete",array('id'=>'incomplete_invoice_form')); ?>
				<?php
				echo "<div class='small_button' id='incomplete_sale_button' style='float:left;margin-top:5px;'><span>Save</span></div>";
				?>
				</form>
				<br /><br /><br /><br />
				<?php echo form_open("lab/complete",array('id'=>'finish_invoice_form')); ?>
        ...
        <div id="Cancel_sale">
		<?php echo form_open("lab/cancel_invoice",array('id'=>'cancel_sale_form')); ?>
...
$(document).ready(function()
{
	...
	var queue_reload = setInterval(function(){$("#queue_section").load('<?php echo site_url("lab/refresh_queue"); ?>');},10000);
    ...
    $("#finish_sale_button").click(function()
    {
     ...
     $("#incomplete_sale_button").click(function()
    {
    	if (confirm('<?php echo $this->lang->line("invoices_confirm_finish_invoice"); ?>'))
    	{
    		$('#incomplete_invoice_form').submit();
    	}
    });
