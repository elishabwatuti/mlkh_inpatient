...
<div id="page_title" style="margin-bottom:8px;"><?php echo $this->lang->line('invoices_xray_request'); ?></div>
<?php
if(isset($error))
{
...
<?php echo form_open("xray_request/add",array('id'=>'add_item_form')); ?>
...
else
{
	foreach(array_reverse($cart, true) as $line=>$item)
	{
		...
		echo form_open("xray_request/edit_item/$line");
	?>
		<tr>
		...
		<td><?php echo anchor("xray_request/delete_item/$line",'['.$this->lang->line('common_delete').']');?></td>
		</tr>
...
<div id="overall_sale">
	...
	if(isset($customer))
	{
		...
		echo anchor("xray_request/remove_customer",'['.$this->lang->line('common_remove').' '.$this->lang->line('customers_customer').']');
	}
	else
	{
		echo form_open("xray_request/select_customer",array('id'=>'select_customer_form')); ?>
	...
	?>
		...
		?>
			<div id="finish_sale">
				<?php echo form_open("xray_request/complete",array('id'=>'finish_invoice_form')); ?>
        ...
        <div id="Cancel_sale">
		<?php echo form_open("xray_request/cancel_invoice",array('id'=>'cancel_sale_form')); ?>
...
{
    $("#item").autocomplete('<?php echo site_url("xray_request/item_search"); ?>',
    {
    	minChars:0,
    	max:100,
    	selectFirst: false,
    ...
    $("#customer").autocomplete('<?php echo site_url("xray_request/customer_search"); ?>',
    {
    	minChars:0,
    	delay:10,
    	max:100,
	...
	{
		$.post('<?php echo site_url("xray_request/set_comment");?>', {comment: $('#comment').val()});
	});
	
	$('#email_receipt').change(function() 
	{
		$.post('<?php echo site_url("xray_request/set_email_receipt");?>', {email_receipt: $('#email_receipt').is(':checked') ? '1' : '0'});
	});
	...
	$("#suspend_sale_button").click(function()
	{
    	...
    	{
			$('#finish_invoice_form').attr('action', '<?php echo site_url("xray_request/suspend"); ?>');
    		$('#finish_invoice_form').submit();
    	}
	});
