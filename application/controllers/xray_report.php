...
class Xray_Report extends Secure_area
{
	function __construct()
	{
		parent::__construct('xray_report');
	...
	function complete()
	{
		...
		$data['invoice_id']='Invoice '.$this->Invoice->save_xray($data['cart'], $customer_id,$employee_id,$comment,$data['total'],$department);
		if ($data['invoice_id'] == 'Invoice -1')
		{
			$data['error_message'] = $this->lang->line('invoices_transaction_failed');
		}
		$this->load->view("xray_report/receipt",$data);
	...
	function _reload($data=array())
	{
		...
		$this->load->view("xray_report/register",$data);
	}
