...
class Lab_Report extends Secure_area
{
	function __construct()
	{
		parent::__construct('lab_report');
	...
	function select_customer($customer_id = 0)
	{
		if ( !$customer_id )
		{
			...
			{ 
				...
				$this->invoice_lib->get_invoice_test($customer_id,$employee_id);
			}
		}
	...
	function edit_item($line)
	{
		...
		if(!$this->invoice_lib->edit_item($line,$result,$serialnumber,$quantity,$discount,$price))
		{
	...
	function complete()
	{
		...
		$data['invoice_id']='Invoice '.$this->Invoice->save_lab($data['cart'], $customer_id,$employee_id,$comment,$data['total'],$department);
		if ($data['invoice_id'] == 'Invoice -1')
		{
			$data['error_message'] = $this->lang->line('invoices_transaction_failed');
		}
		$this->load->view("lab_report/receipt",$data);
		$this->invoice_lib->clear_all();
	}

	function save_incomplete()
	{
		$data['cart']=$this->invoice_lib->get_cart();
		$data['subtotal']=$this->invoice_lib->get_subtotal();
		$data['total']=$this->invoice_lib->get_total();
		...
		//SAVE invoice to database
		$data['invoice_id']='Invoice '.$this->Invoice->save_lab_incomplete($data['cart'], $customer_id,$employee_id,$comment,$data['total'],$department);
		if ($data['invoice_id'] == 'Invoice -1')
		{
			$data['error_message'] = $this->lang->line('invoices_transaction_failed');
		}
	...
	function _reload($data=array())
	{
		...
		$this->load->view("lab_report/register",$data);
	}
